...
class FileScanRDD(
  ...
  override def compute(split: RDDPartition, context: TaskContext): Iterator[InternalRow] = {
    val iterator = new Iterator[Object] with AutoCloseable {
      ...
      /**
       * Helper class for setting the row index metadata column using value compute by the specific
       * file format reader when reading data one record at a time.
       */
      private val rowIndexUpdater =
        RowIndexUtil.getMetadataRowUpdater(readSchema, metadataColumns)

      /**
       * Index of the row index column generated by the specific file format reader.
       * We use it when reading data in batches to add the column to metadata.
       */
